...
-(void) setFee:(NSString *)fee andOrderNo:(NSString *)orderno
{
    totalFee = fee;
    orderNo = orderno;
}
...
- ( NSMutableDictionary *)sendPay
{
    ...
    //订单标题，展示给用户
    NSString *order_name    = @"北京亿街区电子商务有限公司";
    //订单金额,单位（分）
    NSString *order_price   = @"1";//1分钱测试
    if (totalFee) {
        order_price = totalFee;
    }
    ...
    NSString *notify_url = [NSString stringWithFormat:@"%@%@", Service_Address, WX_NOTIFY_URL];
