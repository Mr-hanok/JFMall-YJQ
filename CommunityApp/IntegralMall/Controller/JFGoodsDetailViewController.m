...
//        self.contentViewHeightConstraint.constant = APP_SCREEN_HEIGHT ;
...
- (void)serverApi_FinishedSuccessed:(APIRequest *)api result:(APIResult *)sr {
    ...
    if (api == self.apiAddShopCar) {//加入购物车
        if (self.isCommit) {//点立即兑换
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:sr.msg delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
            [alert show];
        }
