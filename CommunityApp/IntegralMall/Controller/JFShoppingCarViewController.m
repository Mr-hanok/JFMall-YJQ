...
- (IBAction)jieSuanBtnClick:(UIButton *)sender {
    ...
    if ([sender.titleLabel.text isEqualToString:@"删除"]) {//删除
        ...
        if (self.selectArray.count ==0) {
            [HUDManager showWarningWithText:@"请选择要购买的商品!"];
            return;
        }
...
- (void)instalTotalPrice{
    ...
    //给总价文本赋值
    NSString *title1 = [NSString stringWithFormat:@"已选择%ld个",selGoodsNum];
    NSString *title2 = [NSString stringWithFormat:@"合计:%d积分",self.totalIntegral];
    
    if (self.isEdite) {
        NSMutableAttributedString *str1 = [[NSMutableAttributedString alloc]initWithString:title1];
        [str1 addAttribute:NSForegroundColorAttributeName value:HEXCOLOR(0xEA7E36) range:NSMakeRange(3, title1.length-4)];
        ...
        [str2 addAttribute:NSForegroundColorAttributeName value:HEXCOLOR(0xEA7E36) range:NSMakeRange(3, title2.length-5)];
        [self.totalIntegralBtn setAttributedTitle:str2 forState:UIControlStateNormal];
    }
    self.totalIntegral = 0;
}
