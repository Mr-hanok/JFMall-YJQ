...
- (void)initValues
{
    ...
    //加入底问分割线
    bottomLine = [[UIView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView-0.2, self.bounds.size.width, 0.8)];
    ...
    _showBottomValine = NO;
    bottomLine.hidden = !_showBottomValine;
...
- (void)setShowBottomValine:(BOOL)showBottomValine
{
    _showBottomValine = showBottomValine;
    bottomLine.hidden = !_showBottomValine;
}
...
- (void)layoutSubviews
{
    ...
    [self changeRootScrollViewContentOffSet];
}

//设置_rootScrollView的偏移量
- (void)changeRootScrollViewContentOffSet{
    
    _rootScrollView.contentOffset =  CGPointMake(self.index*[UIScreen mainScreen].bounds.size.width, 0);
...
{
    UIButton *selectBtn = [_topScrollView viewWithTag:_userSelectedChannelID + index];
    [self selectNameButton:selectBtn];
}
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [vc.title sizeWithAttributes:@{
                                                         NSFontAttributeName:[UIFont systemFontOfSize:kFontSizeOfTabButton]
                                                         }];
        //累计每个tab文字的长度
        topScrollViewContentWidth += kWidthOfButtonMargin+self.bounds.size.width/_viewArray.count;
        //设置按钮尺寸
        [button setFrame:CGRectMake(0+self.bounds.size.width/_viewArray.count*i,0,
                                    self.bounds.size.width/_viewArray.count, kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        ...
        // 设置阴影图片
        if (i == 0) {
//            _shadowImageView.frame = CGRectMake(0, 0, self.bounds.size.width/_viewArray.count, kHeightOfTopScrollView);
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, self.bounds.size.width/_viewArray.count-2*kWidthOfButtonMargin, kHeightOfTopScrollView);
            button.selected = YES;
        }
        
        //加入button分割线
        if (i<_viewArray.count-1 && self.showBtnValine) {
            UIView *verLineView = [[UIView alloc] initWithFrame:CGRectMake(self.bounds.size.width/_viewArray.count*(i+1), 14, 1, 16)];
            verLineView.backgroundColor = [UIColor colorWithRed:204.0/255 green:204.0/255 blue:204.0/255 alpha:1.0];
            [_topScrollView addSubview:verLineView];
        }
